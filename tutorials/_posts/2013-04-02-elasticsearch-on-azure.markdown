...
For the first one, only VM behind this same virtual port can talk to each other. That means that with this mode, you can use elasticsearch unicast discovery to build a cluster.
...
```sh
# Connect to VM using the 'right' SSH port
ssh -i azurePrivateKey.key elasticsearch@es-vm.cloudapp.net -p 57222
```
...
That's all folks! In fact, do the above steps on each VM and they will discover each other using standard multicast discovery.
